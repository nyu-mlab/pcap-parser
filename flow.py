...
The script filters network traffic for TCP and TLSv1.2, groups packets into flows based on connection details, calculates flow statistics, and extracts initial hostnames. The aggregated data is then saved to a new CSV file.
...
def process_pcap_data(input_csv, output_csv):
    ...
    df = df[df['_ws.col.Protocol'].isin(['TCP', 'TLSv1.2'])] # Filtering out non-IP/TCP/UDP protocols for accurate flow information
    grouped = df.groupby(['ip.src', 'ip.dst', 'tcp.srcport', 'tcp.dstport', '_ws.col.Protocol']) # Grouping by identifiers
